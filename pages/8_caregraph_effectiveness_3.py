...
if 'state2' not in st.session_state:
    ...
    st.session_state.situation2 = (
        "학교 복도에서 급식실로 이동 중, 천장 형광등이 전력 불안정으로 불규칙하게 연속적으로 깜빡이며 이를 바라보던 A가 눈을 가린 채 멜트다운을 일으킴."
    )
    st.session_state.strategy2 = {
        'cause': '복도 천장 형광등의 불규칙한 깜빡임이 예측 불가능한 시각 자극으로 작용해, 평소 광반응 민감도는 낮지만 갑작스러운 패턴 변화에 감각 과부하와 멜트다운을 유발',
        'intervention': [
            {'strategy': '시각 자극 완화',
             'purpose': '불규칙한 깜빡임 빛을 물리적으로 차단해 즉각적 안정감 제공',
             'example': {'immediate': '‘주먹 한번 내밀기’ 제스처 후 가방에서 안경 꺼내 착용 유도·고개 끄덕임이는 바디랭귀지를 통해 “괜찮아” 라는 비언어적 신호 전달',
                         'standard': '매일 등교 직후 1분간 스스로 안경 파우치에서 꺼내 착용 연습·다리 부분 파란색 스티커 부착으로 친숙함 강화'}}
        ]
    }
...
if 'survey8_submitted' not in st.session_state:
    st.session_state.survey8_submitted = False
...
# --- Survey ---
    ...
    if st.button("설문 제출"):
        ...
        st.session_state.survey8_submitted = True    
        st.success("응답이 저장되었습니다. 감사합니다!")

if st.session_state.survey8_submitted:
