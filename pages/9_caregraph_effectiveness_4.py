...
if 'state2' not in st.session_state:
    ...
    st.session_state.situation2 = (
        "가족과 함께 바닷가에 도착해 모래사장을 걸어가던 중, 부모가 별도 예고 없이 “조금만 더 가면 파도가 올 거야”라며 손짓으로 A를 물가 쪽으로 유도했습니다. A는 갑작스럽게 발에 달라붙는 뜨거운 모래와 차가운 바닷물이 동시에 피부에 닿자 움찔하며 손을 떨기 시작했고, 곧 몸을 굳힌 채 큰 소리로 저항하며 Meltdown에 빠졌습니다."
    )
    st.session_state.strategy2 = {
        'cause': '촉각 과부하: 예고 없이 발생한 모래·물의 복합 촉감이 A의 감각 처리 용량을 초과',
        'intervention': [
            {'strategy': '촉각 완충 지원 (Tactile Buffering)',
             'purpose': '과도한 모래·물 촉각 자극을 줄여 A의 신체적·정서적 불안 완화',
             'example': {'immediate': 'A에게 튼튼한 워터 슈즈 또는 모래 차단 매트를 즉시 제공 및 손바닥을 펴서 천천히 바닥에 내려 “여기서 멈춰”라는 제스처와 함께 몸짓으로 안정 신호 전달',
                         'standard': '외출 전 A 전용 워터 슈즈·매트 준비 및 착용 연습 및 모래사장 이동 시 “두 걸음씩 걷기” 같은 일상적 비언어 예고 루틴 정착'}}
        ]
    }
