...
if 'state2' not in st.session_state:
    ...
    st.session_state.situation2 = (
        "가족과 함께 바닷가에 도착해 모래사장을 걸어가던 중, 갑작스럽게 근처 보트가 시동 걸자 큰 굉음이 나며 A가 몸을 웅크린 채 얼굴을 찡그리며 소리를 지름"
    )
    st.session_state.strategy2 = {
        'cause': '갑작스러운 큰 보트 엔진 소음이 A의 청각 처리 용량을 초과하여 과민 반응을 유발',
        'intervention': [
            {'strategy': '청각 완충 지원 (Auditory Buffering) 및 바디랭귀지 소통',
             'purpose': 'A에게 소음 차단용 귀마개나 노이즈 캔슬링 헤드폰을 즉시 착용시켜 주고 부모가 A의 양쪽 어깨를 부드럽게 감싸며 손바닥을 귀 방향으로 가져다 대는 “쉿” 제스처로 안정 신호 전달',
             'example': {'immediate': 'A에게 튼튼한 워터 슈즈 또는 모래 차단 매트를 즉시 제공 및 손바닥을 펴서 천천히 바닥에 내려 “여기서 멈춰”라는 제스처와 함께 몸짓으로 안정 신호 전달',
                         'standard': '외출 전 A 전용 귀마개·헤드폰을 준비하고 집에서 미리 착용 연습을 진행하고 보트나 기타 시끄러운 장비가 가까워질 때마다 같은 “쉿” 제스처와 간단한 시각 카드(🔇 아이콘)로 예고 루틴 실행'}}
        ]
    }
