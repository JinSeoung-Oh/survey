...
if 'state2' not in st.session_state:
    ...
    st.session_state.strategy2 = {
        ...
        'intervention': [
            {'strategy': '캐릭터 테마 소음 차단 헤드폰 제공',
             'purpose': '할머니 생신 파티의 갑작스러운 소음 자극을 즉시 차단하고, 친숙한 캐릭터 디자인으로 심리적 안정감을 부여하여 감각 과부하를 예방',
             'example': {'immediate': '자폐인이 울음을 터뜨리려 할 때 미리 합의된 손 제스처(손바닥 가로 흔들기)를 확인한 뒤, 즉시 캐릭터 테마 헤드폰을 착용시켜주고 착용 후 양 엄지손가락을 들어 올리며 “괜찮아졌어”라는 비언어적 신호(👍)를 보내 안정감을 강화',
                         'standard': '행사 전날과 아침에 집에서 5분간 A 스스로 헤드폰을 꺼내고 착용해 보는 루틴을 매일 연습하고 헤드폰 외부에 자폐인이 좋아하는 스티커를 붙여 ‘내 것’임을 분명히 인식시키고, 익숙함을 높임'}}
        ]
    }
...
설문 조사 완료 후 제출 버튼을 누르셔야 다음 페이지로 이동이 가능합니다.
""")
...
if 'survey6_submitted' not in st.session_state:
    st.session_state.survey6_submitted = False
...
# --- Survey ---
    ...
    if st.button("설문 제출"):
        ...
        st.session_state.survey6_submitted = True
        st.success("응답이 저장되었습니다. 감사합니다!")
        
if st.session_state.survey6_submitted:
